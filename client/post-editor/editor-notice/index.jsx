...
export class EditorNotice extends Component {
	...
	getText( key ) {
		...
		switch ( key ) {
			...
			case 'publishedPrivately':
				...
				if ( 'page' === type ) {
					return translate(
						'Page privately published on {{siteLink/}}! {{a}}Add another page{{/a}}',
						{
							components: {
								siteLink: (
									<a href={ site.URL } target="_blank" rel="noopener noreferrer">{ site.title }</a>
	...
	renderNoticeAction() {
		const { action, link, isPreviewable, onViewClick } = this.props;
	...
	render() {
		...
		return (
			<div className={ classes }>
				...
				{ text &&
					<Notice { ...{ status, text, onDismissClick } } showDismiss={ true }>
