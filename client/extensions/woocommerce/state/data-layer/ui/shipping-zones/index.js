...
} from 'woocommerce/state/sites/shipping-zone-locations/actions';
import {
	actionListStepNext,
...
import {
	getRawShippingZoneLocations,
} from 'woocommerce/state/sites/shipping-zone-locations/selectors';
...
export const getSaveZoneActionListSteps = state => {
	...
	const getZoneStepsFunc = 'number' === typeof zoneId
		? getUpdateShippingZoneSteps
		: getCreateShippingZoneSteps;
	...
	return [
		...
		...flatten(
			methodEdits.creates.map( method =>
				getZoneMethodCreateSteps( siteId, zoneId, method, state ),
			),
		),
	];
};
...
export default {
	[ WOOCOMMERCE_SHIPPING_ZONE_ACTION_LIST_CREATE ]: [
		( store, action ) => {
			...
			const nextSteps = ( deleteZone ? getDeleteZoneActionListSteps : getSaveZoneActionListSteps )(
				store.getState(),
			);
