...
module.exports = {
	...
	buildUpdateTemplate: function() {
		var post = this.props.post || this.props.page,
			...
			updateText, undoTemplate, undoClick, trashText;
		...
		switch( status ) {
			...
			case 'deleting':
				...
				updateText = ( <span>{ trashText } <span className="loading-dot">.</span><span className="loading-dot">.</span><span className="loading-dot">.</span></span> );
			...
			case 'deleted':
				...
				break;
			case 'updating':
				updateText = ( <span>{ strings.updating } <span className="loading-dot">.</span><span className="loading-dot">.</span><span className="loading-dot">.</span></span> );
			...
			case 'error':
				...
				break;
			case 'restoring':
				updateText = ( <span>{ strings.restoring } <span className="loading-dot">.</span><span className="loading-dot">.</span><span className="loading-dot">.</span></span> );
				updateClass += ' conf-alert--updating';
				break;
			default:
	...
	updatePostStatus: function( status ) {
		...
		if ( status === 'delete' ) {
			...
			actions.update( post, { status: status }, function( error, post ) {
